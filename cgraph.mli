...
(** [undo_info] holds information useful to perform limited backtracking of certain
    computations. See {!undo}. *)
type undo_info

(** [Var] is the module type of variables, which are the simplest kind
...
module Var : sig
  ...
  (** [create_tracking node] creates a variable that tracks the value of [node].
  ...
  (** [set_with_undo v x] behaves as {!set}. However, it returns
      a value of type {!undo_info} holding information allowing to
      revert the effect of the assignement. *)
...
(** [get n] computes the current value associated to [n]. This might
     recursively trigger the recomputation of all currently invalidated node
     on which [n] depends. *)
val get : ?u:undo_info -> 'a t -> 'a
...
(** [undo] allows to revert the underlying state of a value after having
    performed a call to {!Var.set_with_undo} followed by a call to {!get}.
    Note that some side-effects that cannot be observed by the user might
    not be reverted.

    Interleaving other calls to the public API might result in undefined
    behaviour.
...
module Internal : sig
  ...
  val add_used_by : 'a t -> ex -> unit

  val set_next_debug_info : string -> unit
end
